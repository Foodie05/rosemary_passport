...
create unique index if not exists idx_legal_one_draft_per_type on legal_documents(document_type) where status = 'draft';
create index if not exists idx_legal_published on legal_documents(document_type, version desc) where status = 'published';
...
create index if not exists idx_legal_acceptance_user on user_legal_acceptances(user_id, accepted_at desc);
...
create index if not exists idx_activity_category_date on activity_logs(category, created_at desc);
