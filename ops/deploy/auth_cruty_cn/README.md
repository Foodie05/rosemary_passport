...
4. 为备份桶启用版本控制和至少 30 天的 Object Lock 默认保留策略。部署预检会拒绝未满足配置要求的桶。仅当对象存储官方明确不支持对应能力且已有书面风险例外时，才可将 `S3_REQUIRE_VERSIONING=false` 或 `S3_REQUIRE_OBJECT_LOCK=false`；此时仍强制使用唯一时间戳对象名、客户端加密和上传后完整性校验。
5. 执行：
...
首次从旧 Compose 切换时，旧 `.env` 仅在两类异地备份成功后移入
`/etc/rosm-passport/legacy-env/` 的 root-only 回滚隔离区；失败会自动恢复。稳定观察和
回滚窗口结束后再受控销毁该副本。
容器镜像按发布时间生成不可变标签，旧代码与旧标签一起进入无凭据本地快照；构建在
切换前完成，切换后的任一失败都会恢复旧代码、旧环境、旧标签和旧 Compose 服务。
CI 会直接执行完整发布事务夹具，覆盖正常切换、启动前失败和 readiness 失败；每条路径都
核对运行配置、镜像标签、前端、代码、数据库目录和本地备份是否处于预期状态。

默认发布容量预检要求至少 2 个 CPU，并保留 5 GiB 磁盘余量。部署不执行物理内存、
可用内存或 Swap 容量门禁；运行期资源是否足够由 readiness、冒烟测试和稳定观察判断。
...
- `LEGACY_JSON_REFRESH_SUNSET_AT` 的占位值在首次加固启动前原子替换为当时 UTC 时间加
  14 天；后续部署检测到固定时间后保持不变，不得滚动延后。到期后 JSON Refresh Token
  自动拒绝，Refresh Cookie 不受影响。
