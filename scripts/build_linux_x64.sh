...
cp "$TEMPLATE_DIR/s3_key.sh" "$RELEASE_DIR/s3_key.sh"
cp "$TEMPLATE_DIR/ed25519_signature.sh" "$RELEASE_DIR/ed25519_signature.sh"
...
chmod +x "$RELEASE_DIR/deploy.sh" "$RELEASE_DIR/backup_to_s3.sh" \
  ...
  "$RELEASE_DIR/s3_key.sh" \
  "$RELEASE_DIR/ed25519_signature.sh" \
