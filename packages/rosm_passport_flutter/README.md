...
The built-in UI supports email code login, phone code login, password login, password recovery, passkey login, email registration, and the final consent page. Registration is enabled by default. Set `enableRegistration: false` if an app wants to hide account creation.
